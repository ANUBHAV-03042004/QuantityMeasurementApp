...
public class QuantityMeasurementEntity implements Serializable {
    ...
    public String toString() {
        if (hasError) return String.format("[%s] %-12s | ERROR: %s", timestamp, operationType, errorMessage);
        if ("COMPARE".equals(operationType)) return String.format("[%s] %-12s | %s == %s => %b", timestamp, operationType, operand1, operand2, comparisonResult);
        if (operand2 == null) return String.format("[%s] %-12s | %s => %s", timestamp, operationType, operand1, result);
