...
public class Quantity<U extends Enum<U> & IMeasurable> {
    ...
    public Quantity(double value, U unit) {
        if (unit == null)
            throw new IllegalArgumentException("Unit cannot be null in Quantity");
        this.value = value;
        this.unit  = unit;
    }
    ...
    public int hashCode() {
        ...
        double baseValue = unit.convertToBaseUnit(value);
        return Double.hashCode(Math.round(baseValue * 1e9) / 1e9);
    }
    ...
    public Quantity<U> add(Quantity<U> other) {
        ...
        double sumInUnit = unit.convertFromBaseUnit(sumBase);
        return new Quantity<>(sumInUnit, unit);
    }

   
    public Quantity<U> subtract(Quantity<U> other) {
        unit.validateOperationSupport("SUBTRACT");
        double diffBase   = unit.convertToBaseUnit(value)
                          - other.unit.convertToBaseUnit(other.value);
        double diffInUnit = unit.convertFromBaseUnit(diffBase);
        return new Quantity<>(diffInUnit, unit);
    }
